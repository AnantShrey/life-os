...
export function WatchlistCard({ item }: { item: Item }) {
    ...
    <div className="group bg-card rounded-[20px] p-4 flex gap-4 transition-all hover:-translate-y-0.5"
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          {item.user_rating && <span className="text-xs text-muted-foreground">You: {item.user_rating}/10</span>}
        </div>

        {/* TV progress */}
        {item.media_type === "tv" && status === "watching" && (
          editProgress ? (
        ...
        <div className="flex items-center gap-2 mt-auto">
          <select value={status} onChange={e=>updateStatus(e.target.value as any)} disabled={pending}
            className={`text-xs font-medium px-2.5 py-1 rounded-full border-0 cursor-pointer appearance-none ${STATUS_STYLES[status]}`}>
      ...
      <button disabled={pending}
        ...
        className="self-start opacity-0 group-hover:opacity-100 p-1.5 text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-[8px] transition-all">
